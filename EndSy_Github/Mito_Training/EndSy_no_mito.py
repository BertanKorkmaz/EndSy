...
class Environment: 
    ...
    def iterate(self):
        ...
        current_increment = self.daily_increment if self.iteration < self.daily_increment_iterations else 0
        for i in range(self.height): 
            for j in range(self.width): 
                #buradaki amaç gridteki her birimi satır ve sütün dolaşıp az sonra onlara capacity'i geçmeyecek şekilde daily increment ile besin eklemek
                #burda bir de min fonksiyonu ile capacity'yi geçmiyor eklenecek besinle beraber çünkü min fonksiyonu içerdekinin en azını aldırtır çıktı olarak
                self.grid[i][j] = min(self.capacity, self.grid[i][j] + current_increment)
                       
        self.apply_external_nutrients() #ek besin eklenmesi
        
        self.diffuse() #difizyonu çağırdık
        
        self.iteration += 1   # İterasyon sayacını arttır
        
        if self.iteration % self.save_interval == 0: 
            self.save_snapshot() # Eğer iterasyon sayısı save_interval'in katı ise, snapshot kaydet. Amaç: Mesela 5 iterasyonda 1 kaydet demek için.
...
class ProkaryoticCell:
    ...
    def __init__(self, x, y, birth_iteration=0, dysregulation_params=None, dysregulation_threshold_interval = (0.85,1),
        ...
        # --- Kanser indeksi hesaplaması için ---
...
env = Environment(width=50, height=50, capacity=5, daily_increment=0.5, daily_increment_iterations=100)
